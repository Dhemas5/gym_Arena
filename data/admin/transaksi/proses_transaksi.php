...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_single_item') {
    ...
    try {
        ...
        // === OTOMATIS AKTIFKAN MEMBERSHIP (jika member) ===
        if ($id_member !== null) {
            // Ambil durasi paket
            ...
            $tgl_berakhir = date('Y-m-d', strtotime("+$durasi_hari days", strtotime($tgl_mulai)));

            // Cek apakah ini perpanjangan
            $jenis_aktivasi = 'Baru';
            $cek_last = $con->prepare("
                SELECT tgl_berakhir FROM tbl_aktivasi_membership 
                WHERE id_member = ? 
                ORDER BY tgl_dicatat DESC LIMIT 1
            ");
            ...
            // INSERT ke tbl_aktivasi_membership
            ...
            if (!$stmt_aktivasi->execute()) {
                throw new Exception("Gagal simpan aktivasi membership: " . $stmt_aktivasi->error);
            }
            $stmt_aktivasi->close();

            // UPDATE tbl_member
            $sql_update = "UPDATE tbl_member 
                SET status_membership = 'Aktif',
                    id_paket_aktif = ?,
                    tgl_mulai_aktif = ?,
                    tgl_kedaluwarsa = ?
                WHERE id_member = ?";
            ...
            if (!$stmt_update->execute()) {
                throw new Exception("Gagal update status member: " . $stmt_update->error);
            }
            $stmt_update->close();
        }
